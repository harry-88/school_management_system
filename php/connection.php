...
function createTables(){
    ...
    $query = "CREATE TABLE IF NOT EXISTS Teachers(teacher_id int auto_increment primary key, first_name varchar(100), last_name varchar(100), mobile_number varchar(100),cnic varchar(100),address varchar(100), gender varchar (100), qualification varchar(100), subject varchar(100),joining_date varchar(100), email varchar(100) unique, password varchar (100),account_status boolean)";
    mysqli_query($connection,$query);
    

    $query = "CREATE TABLE IF NOT EXISTS Classes(class_name varchar(100) primary key, section char(100),incharge_id int, foreign key(incharge_id) references teachers(teacher_id))";
    mysqli_query($connection,$query);

    $query = "CREATE TABLE IF NOT EXISTS Students(student_id int auto_increment, first_name varchar(100), last_name varchar(100),class_name varchar(100), mobile_number varchar(100),cnic varchar(100),address varchar(100), gender varchar (100),blood_group char(3),email varchar(100) unique,password varchar (100),parent_id int,account_status boolean, primary key(student_id), foreign key(class_name) references classes(class_name), foreign key(parent_id) references Parents(parent_id))";
    mysqli_query($connection,$query);

    $query = "CREATE TABLE IF NOT EXISTS Subjects(subject_id int primary key auto_increment, subject_title varchar(100),subject_type varchar(100),class_name varchar(100),foreign key(class_name) references Classes(class_name))";
    mysqli_query($connection,$query);
    ...
    $query = "CREATE TABLE IF NOT EXISTS assessments(assessment_id int primary key AUTO_INCREMENT,student_id int,assessment_name varchar(100),subject_title varchar(100),totla_marks int,obtained_marks int,date varchar(100),FOREIGN KEY(student_id) REFERENCES Students(student_id))";
    mysqli_query($connection,$query);
